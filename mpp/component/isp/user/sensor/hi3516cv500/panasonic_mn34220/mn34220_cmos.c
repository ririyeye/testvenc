...
static HI_S32 cmos_fps_set_None_LINE(VI_PIPE ViPipe, HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (pstSnsState->u8ImgMode == MN34220_1080P_30FPS_MODE) {
    ...
    } else if (pstSnsState->u8ImgMode == MN34220_720P_30FPS_MODE) {
    ...
    } else if (pstSnsState->u8ImgMode == MN34220_720P_120FPS_MODE) {
    ...
    } else if (pstSnsState->u8ImgMode == MN34220_VGA_240FPS_MODE) {
        ...
        ISP_ERR_TRACE("Not support! u8ImgMode:%d, f32Fps:%f\n", pstSnsState->u8ImgMode, f32Fps);
        return HI_FAILURE;
    }
...
static HI_VOID cmos_fps_set(VI_PIPE ViPipe, HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    } else if (pstSnsState->enWDRMode == WDR_MODE_4To1_LINE) {
                                       ...
                                       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    } else if ((pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) || (pstSnsState->enWDRMode == WDR_MODE_2To1_FRAME_FULL_RATE) || (pstSnsState->enWDRMode == WDR_MODE_2To1_FRAME)) {
...
static HI_VOID cmos_inttime_update(VI_PIPE ViPipe, HI_U32 u32IntTime)
{
    ...
    if ((pstSnsState->enWDRMode == WDR_MODE_2To1_FRAME_FULL_RATE) || (pstSnsState->enWDRMode == WDR_MODE_2To1_FRAME)) {
        ...
        if (u8Count[ViPipe] == 0) { /* short exposure */
            ...
            u8Count[ViPipe]++;
        } else if (u8Count[ViPipe] == 1) { /* long exposure */
            pstSnsState->au32WDRIntTime[1] = u32IntTime;
            pstSnsState->astRegsInfo[0].astI2cData[7].u32Data = ((u32IntTime & 0xFF00) >> 8);
            pstSnsState->astRegsInfo[0].astI2cData[8].u32Data = (u32IntTime & 0xFF);
            u8Count[ViPipe]++;
        ...
        if (u8Count[ViPipe] == 0) { /* short short exposure */
            pstSnsState->au32WDRIntTime[0] = u32IntTime;
            ...
            u8Count[ViPipe]++;
        } else if (u8Count[ViPipe] == 1) { /* short exposure */
            pstSnsState->au32WDRIntTime[1] = u32IntTime;
            pstSnsState->astRegsInfo[0].astI2cData[9].u32Data = ((u32IntTime & 0xFF00) >> 8);
            pstSnsState->astRegsInfo[0].astI2cData[10].u32Data = (u32IntTime & 0xFF);
            u8Count[ViPipe]++;
        } else if (u8Count[ViPipe] == 2) { /* long exposure */
            pstSnsState->au32WDRIntTime[2] = u32IntTime;
            pstSnsState->astRegsInfo[0].astI2cData[7].u32Data = ((u32IntTime & 0xFF00) >> 8);
            pstSnsState->astRegsInfo[0].astI2cData[8].u32Data = (u32IntTime & 0xFF);
            u8Count[ViPipe]++;
...
static HI_S32 cmos_get_inttime_max_temp(HI_U32 *pu32IntTimeMaxTmp, VI_PIPE ViPipe, HI_U32 *au32Ratio, HI_U32 *au32IntTimeMax, HI_U32 *au32IntTimeMin, HI_U32 *pu32LFMaxIntTime)
{
    ...
    } else if ((pstSnsState->enWDRMode == WDR_MODE_3To1_FRAME_FULL_RATE) || (pstSnsState->enWDRMode == WDR_MODE_3To1_FRAME)) {
        *pu32IntTimeMaxTmp = ((pstSnsState->au32FL[0] - 2) << 12) / (DIV_0_TO_1(au32Ratio[0]) * DIV_0_TO_1(au32Ratio[1]));
    } else if ((pstSnsState->enWDRMode == WDR_MODE_4To1_FRAME_FULL_RATE) || (pstSnsState->enWDRMode == WDR_MODE_4To1_FRAME)) {
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((pstSnsState->bSyncInit == HI_FALSE) || (pstSnsRegsInfo->bConfig == HI_FALSE)) {
        ...
        } else if ((pstSnsState->enWDRMode == WDR_MODE_2To1_FRAME_FULL_RATE) || (pstSnsState->enWDRMode == WDR_MODE_2To1_FRAME)) {
            pstSnsState->astRegsInfo[0].u32RegNum += 2;
        } else if ((pstSnsState->enWDRMode == WDR_MODE_3To1_LINE) || (pstSnsState->enWDRMode == WDR_MODE_3To1_FRAME_FULL_RATE) || (pstSnsState->enWDRMode == WDR_MODE_3To1_FRAME)) {
            pstSnsState->astRegsInfo[0].u32RegNum += 4;
        } else if ((pstSnsState->enWDRMode == WDR_MODE_4To1_LINE) || (pstSnsState->enWDRMode == WDR_MODE_4To1_FRAME_FULL_RATE) || (pstSnsState->enWDRMode == WDR_MODE_4To1_FRAME)) {
...
static HI_S32 cmos_set_image_mode_720P(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode, HI_U8 *pu8SensorImageMode)
{
    ...
    } else if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) { /* for FPGA */
        if (pstSensorImageMode->f32Fps <= 30) {
            *pu8SensorImageMode = MN34220_720P_30FPS_MODE;
        } else {
            ISP_ERR_TRACE("Not support! Width:%d, Height:%d, Fps:%f, WDRMode:%d\n",
    ...
    } else if (pstSnsState->enWDRMode == WDR_MODE_3To1_LINE) { /* for FPGA */
    ...
    } else if (pstSnsState->enWDRMode == WDR_MODE_4To1_LINE) { /* for FPGA */
...
static HI_S32 cmos_set_image_mode_1080P(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode, HI_U8 *pu8SensorImageMode)
{
    ...
    if ((pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) || (pstSnsState->enWDRMode == WDR_MODE_3To1_LINE) || (pstSnsState->enWDRMode == WDR_MODE_4To1_LINE)) {
        if (pstSensorImageMode->f32Fps <= 30) {
