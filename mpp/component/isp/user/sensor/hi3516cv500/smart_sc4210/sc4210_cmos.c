...
static HI_U32 g_au32Sc4210Ratio[ISP_MAX_PIPE_NUM] = {[0 ...(ISP_MAX_PIPE_NUM - 1)] = 64};
static HI_U16 g_au16Sc4210ShortFrameExpMin[ISP_MAX_PIPE_NUM] = {0};

static HI_U32 g_u32Fps[ISP_MAX_PIPE_NUM] = {[0 ...(ISP_MAX_PIPE_NUM - 1)] = 30};
...
#define SC4210_MAX_SHORT_EXP        (0xFFF)
...
static HI_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (g_au32LinesPer500ms[ViPipe] == 0) {
        pstAeSnsDft->u32LinesPer500ms = pstSnsState->u32FLStd * U32MaxFps / 2;
    } else {
    ...
    switch (pstSnsState->enWDRMode) {
        ...
        case WDR_MODE_2To1_LINE:
            ...
            pstAeSnsDft->u8AeCompensation = 20;

            pstAeSnsDft->stIntTimeAccu.f32Accuracy = 2;
            pstAeSnsDft->u32MaxIntTime = pstSnsState->u32FLStd - 8;
            pstAeSnsDft->u32MinIntTime = 2;//2;//192;
            g_au16Sc4210ShortFrameExpMin[ViPipe] = pstAeSnsDft->u32MinIntTime;
            pstAeSnsDft->u32MaxIntTimeTarget = 65535;
...
static HI_VOID cmos_fps_set(VI_PIPE ViPipe, HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    switch (pstSnsState->u8ImgMode) {
        case SC4210_SENSOR_1440P_30FPS_LINEAR_MODE:
            ...
            pstAeSnsDft->u32LinesPer500ms = SC4210_VMAX_1440P30_LINEAR*15;

            break;

        case SC4210_SENSOR_1440P_30FPS_WDR_MODE:
            if ((f32Fps <= 30) && (f32Fps >= 15.0)) {
                u32VMAX = SC4210_VMAX_1440P30_WDR * 30 / DIV_0_TO_1_FLOAT(f32Fps);
            } else {
                ISP_ERR_TRACE("Not support Fps: %f\n", f32Fps);
                return;
            }
            pstAeSnsDft->u32LinesPer500ms = SC4210_VMAX_1440P30_WDR*15;
            break;
    ...
    g_au16Sc4210VMAX[ViPipe] = u32VMAX;
...
static HI_VOID cmos_slow_framerate_set(VI_PIPE ViPipe, HI_U32 u32FullLines, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        u32VMAX = (u32VMAX > (SC4210_VMAX_1440P30_WDR * 2)) ? (SC4210_VMAX_1440P30_WDR * 2) : u32VMAX; //The min fps is 3
...
static HI_VOID cmos_inttime_update(VI_PIPE ViPipe, HI_U32 u32IntTime)
{
    ...
    static HI_U32 au32ShortIntTime[ISP_MAX_PIPE_NUM] = {0};
    static HI_U32 au32LongIntTime[ISP_MAX_PIPE_NUM] = {0};

    HI_U32 u32ShortExpLimit = 0xb4;      //Init value is same to the register initial vlaue
    HI_U32 u32LongExpLimit = 0xb4;
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        if (au8ExpFrameType[ViPipe] == SC4210_FRAME_TYPE_SHORT) { /* short exposure */
            pstSnsState->au32WDRIntTime[0] = u32IntTime;
            au32ShortIntTime[ViPipe] = u32IntTime;

            au8ExpFrameType[ViPipe] = SC4210_FRAME_TYPE_LONG;

        } else { /* long exposure */
            pstSnsState->au32WDRIntTime[1] = u32IntTime;
            au32LongIntTime[ViPipe] = u32IntTime;

            u32ShortExpLimit = (pstSnsState->u32FLStd - 8) * 64 / (g_au32Sc4210Ratio[ViPipe] + 64);
            u32LongExpLimit = (pstSnsState->u32FLStd - 8) - u32ShortExpLimit;

            //down use even number
            u32ShortExpLimit = u32ShortExpLimit - (u32ShortExpLimit % 2);

            if (au32LongIntTime[ViPipe] > u32LongExpLimit) {
                au32LongIntTime[ViPipe] = u32LongExpLimit;
            }

            if (au32ShortIntTime[ViPipe] > (u32ShortExpLimit - 2)) {
                au32ShortIntTime[ViPipe] = (u32ShortExpLimit - 2);
            }

            if (au32LongIntTime[ViPipe] > (pstSnsState->u32FLStd - 8 - u32ShortExpLimit)) {
                au32LongIntTime[ViPipe] = (pstSnsState->u32FLStd - 8 - u32ShortExpLimit);
            }

            if (au32ShortIntTime[ViPipe] > 2) {
                au32LongIntTime[ViPipe]  = au32LongIntTime[ViPipe] - (au32LongIntTime[ViPipe] % 2);
                au32ShortIntTime[ViPipe] = au32ShortIntTime[ViPipe] - (au32ShortIntTime[ViPipe] % 2);
            }

            if (au32ShortIntTime[ViPipe] > au32LongIntTime[ViPipe]) {
                au32ShortIntTime[ViPipe] = au32LongIntTime[ViPipe];
            }
            au32LongIntTime[ViPipe]  = au32LongIntTime[ViPipe] << 1;
            au32ShortIntTime[ViPipe] = au32ShortIntTime[ViPipe] << 1;

            if (au32ShortIntTime[ViPipe] > 0xFFF) {
                au32ShortIntTime[ViPipe] = 0xFFF;
            }
            pstSnsState->astRegsInfo[0].astI2cData[0].u32Data = SC4210_EXP_LOW_4BITS(au32LongIntTime[ViPipe]);
            pstSnsState->astRegsInfo[0].astI2cData[1].u32Data = SC4210_EXP_MID_8BITS(au32LongIntTime[ViPipe]);
            pstSnsState->astRegsInfo[0].astI2cData[2].u32Data = SC4210_EXP_HIG_4BITS(au32LongIntTime[ViPipe]);

            pstSnsState->astRegsInfo[0].astI2cData[9].u32Data = SC4210_EXP_LOW_4BITS(au32ShortIntTime[ViPipe]);
            pstSnsState->astRegsInfo[0].astI2cData[10].u32Data = SC4210_EXP_MID_8BITS(au32ShortIntTime[ViPipe]);

            pstSnsState->astRegsInfo[0].astI2cData[16].u32Data = LOW_8BITS(u32ShortExpLimit);
            pstSnsState->astRegsInfo[0].astI2cData[17].u32Data = HIGH_8BITS(u32ShortExpLimit);
            
            au8ExpFrameType[ViPipe] = SC4210_FRAME_TYPE_SHORT;
        }
    } else {
        u32Value = u32IntTime << 1;

        pstSnsState->astRegsInfo[0].astI2cData[0].u32Data = SC4210_EXP_LOW_4BITS(u32Value);
        pstSnsState->astRegsInfo[0].astI2cData[1].u32Data = SC4210_EXP_MID_8BITS(u32Value);
        pstSnsState->astRegsInfo[0].astI2cData[2].u32Data = SC4210_EXP_HIG_4BITS(u32Value);
    }
...
static HI_VOID cmos_gains_update(VI_PIPE ViPipe, HI_U32 u32Again, HI_U32 u32Dgain)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        pstSnsState->astRegsInfo[0].astI2cData[12].u32Data = LOW_8BITS(u32Again);
        pstSnsState->astRegsInfo[0].astI2cData[13].u32Data = HIGH_8BITS(u32Again);

        pstSnsState->astRegsInfo[0].astI2cData[14].u32Data = LOW_8BITS(u32Dgain);
        pstSnsState->astRegsInfo[0].astI2cData[15].u32Data = HIGH_8BITS(u32Dgain);
    }
...
static HI_VOID cmos_get_inttime_max(VI_PIPE ViPipe, HI_U16 u16ManRatioEnable, HI_U32 *au32Ratio, HI_U32 *au32IntTimeMax, HI_U32 *au32IntTimeMin, HI_U32 *pu32LFMaxIntTime)
{
    ...
    HI_U32 u32IntTimeLongPreUnit = 0;
    HI_U32 u32IntTimeShortCurUnit = 0;

    HI_U32 u32IntTimeMaxTmp  = 0;
    HI_U32 u32ShortTimeMinLimit = 2;
    ...
    u32ShortTimeMinLimit = g_au16Sc4210ShortFrameExpMin[ViPipe];

    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        g_au32Sc4210Ratio[ViPipe] = *au32Ratio;

        if (genFSWDRMode[ViPipe] == ISP_FSWDR_LONG_FRAME_MODE) {
            u32IntTimeLongPreUnit = pstSnsState->au32FL[1] - 6 - pstSnsState->au32WDRIntTime[0];
            u32IntTimeShortCurUnit = pstSnsState->au32FL[0] - 10;

            u32IntTimeMaxTmp = (u32IntTimeLongPreUnit < u32IntTimeShortCurUnit) ? u32IntTimeLongPreUnit : u32IntTimeShortCurUnit;
            au32IntTimeMax[0] = u32IntTimeMaxTmp;
            au32IntTimeMin[0] = u32ShortTimeMinLimit;
            return;
        } else {

            u32IntTimeLongPreUnit = ((pstSnsState->au32FL[1] - 8 - pstSnsState->au32WDRIntTime[0]) * 0x40)  / DIV_0_TO_1(au32Ratio[0]);
            u32IntTimeShortCurUnit  = ((pstSnsState->au32FL[0] - 8) * 0x40)  / DIV_0_TO_1(au32Ratio[0] + 0x40);

            u32IntTimeMaxTmp  = (u32IntTimeLongPreUnit < u32IntTimeShortCurUnit) ? u32IntTimeLongPreUnit : u32IntTimeShortCurUnit;        //Use the min
...
static HI_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, HI_BOOL bEnable)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ISP_ERR_TRACE("Please use linear mode to do defetc pixel detect!\n");
        return;
    }
...
static HI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, HI_U8 u8Mode)
{
    ...
    switch (u8Mode & 0x3F) {
        case WDR_MODE_NONE:
            ...
            ISP_ERR_TRACE("linear mode\n");
            break;

        case WDR_MODE_2To1_LINE:
            pstSnsState->enWDRMode = WDR_MODE_2To1_LINE;
            ISP_ERR_TRACE("wdr mode\n");
            break;
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((pstSnsState->bSyncInit == HI_FALSE) || (pstSnsRegsInfo->bConfig == HI_FALSE)) {
        ...
        if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
            pstSnsState->astRegsInfo[0].u32RegNum = 18;
        }
        ...
        if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[9].u32RegAddr = SC4210_EXP_SHORT_L_ADDR;     //short frame exposure
            pstSnsState->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[10].u32RegAddr = SC4210_EXP_SHORT_H_ADDR;

            pstSnsState->astRegsInfo[0].astI2cData[12].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[12].u32RegAddr = SC4210_AGAIN_SHORT_L_ADDR;  //short frame Again
            pstSnsState->astRegsInfo[0].astI2cData[13].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[13].u32RegAddr = SC4210_AGAIN_SHORT_H_ADDR;

            pstSnsState->astRegsInfo[0].astI2cData[14].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[14].u32RegAddr = SC4210_DGAIN_SHORT_L_ADDR;  //short frame Dgain
            pstSnsState->astRegsInfo[0].astI2cData[15].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[15].u32RegAddr = SC4210_DGAIN_SHORT_H_ADDR;

            pstSnsState->astRegsInfo[0].astI2cData[16].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[16].u32RegAddr = SC4210_MAX_SHORT_EXP_L_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[17].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[17].u32RegAddr = SC4210_MAX_SHORT_EXP_H_ADDR;
    ...
    if (pstSensorImageMode->f32Fps <= 30) {
        if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
            if (SC4210_RES_IS_1440P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
                ...
                ISP_ERR_TRACE("Linear Not support! Width:%d, Height:%d, Fps:%f, WDRMode:%d\n",
            ...
            if (SC4210_RES_IS_1440P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
                ...
                ISP_ERR_TRACE("WDR Not support! Width:%d, Height:%d, Fps:%f, WDRMode:%d\n",
                          pstSensorImageMode->u16Width,
                          pstSensorImageMode->u16Height,
                          pstSensorImageMode->f32Fps,
                          pstSnsState->enWDRMode);
