...
static HI_VOID cmos_inttime_update(VI_PIPE ViPipe, HI_U32 u32IntTime)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        if (bFirst[ViPipe]) { /* short exposure */
            ...
            if (gu32STimeFps == 30) {
                if ((au32LongIntTime[ViPipe] >= 750) && (au32ShortIntTime[ViPipe] < 750) && ((au32LongIntTime[ViPipe] + 750) < 2450) && (gf32AGain >= 11)) {
                    fCGain = gf32AGain * gf32DGain * au32ShortIntTime[ViPipe] / 750;
                    if (fCGain < 16) {
                        u32Again = fCGain * 128;
                        u32Dgain = 1024;
                    } else if (fCGain >= 16) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[8].u32Data = LOW_8BITS(au32ShortIntTime[ViPipe]);
            pstSnsState->astRegsInfo[0].astI2cData[9].u32Data = HIGH_8BITS(au32ShortIntTime[ViPipe]);
...
static HI_VOID cmos_gains_update(VI_PIPE ViPipe, HI_U32 u32Again, HI_U32 u32Dgain)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        if (u8ExpFrame[ViPipe] == OS05A_FRAME_TYPE_LONG) {
            ...
            u8ExpFrame[ViPipe] = OS05A_FRAME_TYPE_SHORT;
        } else if (u8ExpFrame[ViPipe] == OS05A_FRAME_TYPE_SHORT) {
            pstSnsState->astRegsInfo[0].astI2cData[10].u32Data = LOW_8BITS(u32Again);
            pstSnsState->astRegsInfo[0].astI2cData[11].u32Data = ((u32Again & 0x3F00) >> 8);
            pstSnsState->astRegsInfo[0].astI2cData[12].u32Data = LOW_8BITS(u32Dgain);
            pstSnsState->astRegsInfo[0].astI2cData[13].u32Data = ((u32Dgain & 0x3F00) >> 8);
            u8ExpFrame[ViPipe] = OS05A_FRAME_TYPE_LONG;
        } else {
        }
    } else {
