...
static HI_VOID CaRegsInitialize(VI_PIPE ViPipe, isp_reg_cfg *pstRegCfg)
{
    ...
    for (i = 0; i < pstRegCfg->cfg_num; i++) {
        pstRegCfg->alg_reg_cfg[i].stCaRegCfg.bCaEn      = bIsOfflineMode ? (pstCA->bCaEn) : (HI_FALSE);
        pstRegCfg->alg_reg_cfg[i].stCaRegCfg.bLut2SttEn = HI_TRUE;
...
static HI_S32 CaInInitialize(VI_PIPE ViPipe)
{
    ...
    if (sns_dft->key.bit1_ca) {
        ...
        memcpy(pstCA->au16YRatioLut, sns_dft->ca->y_ratio_lut, HI_ISP_CA_YRATIO_LUT_LENGTH * sizeof(HI_U16));
        memcpy(pstCA->as16CaIsoRatio, sns_dft->ca->iso_ratio, ISP_AUTO_ISO_STRENGTH_NUM * sizeof(HI_S16));
    } else {
        pstCA->bCaEn  = HI_TRUE;
...
static HI_VOID Isp_Ca_Usr_Fw(ISP_CA_S *pstCA, ISP_CA_USR_CFG_S *pstUsrRegCfg)
{
    ...
    for (j = 0; j < HI_ISP_CA_YRATIO_LUT_LENGTH; j++) {
        pstUsrRegCfg->au16YRatioLUT[j] = MIN2((HI_S32)pstCA->au16YRatioLut[j] * pstCA->s16SaturationRatio / 1000, HI_ISP_CA_RATIO_MAX);
    }
...
HI_S32 isp_alg_register_ca(VI_PIPE ViPipe)
{
    ...
    ISP_GET_CTX(ViPipe, pstIspCtx);
    ISP_ALG_CHECK(pstIspCtx->alg_key.bit1_ca);
