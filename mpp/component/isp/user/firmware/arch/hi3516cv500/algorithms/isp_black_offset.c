...
static HI_VOID BlcInitialize(VI_PIPE ViPipe)
{
    ...
    for (i = 0; i < ISP_BAYER_CHN_NUM; i++) {
        pstBlackLevel->au16BlackLevel[i] = sns_black_level->black_level[i];
    }
...
static HI_VOID GetBuiltInExpanderBlc(const hi_isp_cmos_expander *sns_expander, HI_U16 *pu16SensorBlc, HI_U16 *pu16ExpanderBlc)
{
    HI_U8  i, j;
    ...
    for (i = 1; i < ISP_EXPANDER_POINT_NUM + 1; i++) {
        au16X[i] = sns_expander->expander_point[i - 1].x;
        au32Y[i] = sns_expander->expander_point[i - 1].y;
    }
...
static HI_S32 BE_BlcDynaRegs_BuiltIn(VI_PIPE ViPipe, ISP_BE_BLC_CFG_S  *pstBeBlcCfg, ISP_BLACKLEVEL_S *pstBlackLevel)
{
    ...
    if (sns_dft->key.bit1_expander) {
        ...
        GetBuiltInExpanderBlc(sns_dft->expander, pstBlackLevel->au16RmDiffBlackLevel, au16BlackLevel);
        GetBuiltInExpanderBlc(sns_dft->expander, pstBlackLevel->au16BlackLevel, au16ActualBlackLevel);
    } else {
        for (i = 0; i < ISP_BAYER_CHN_NUM; i++) {
...
static HI_VOID BlcRegsInitialize(VI_PIPE ViPipe, isp_reg_cfg *pstRegCfg)
{
    HI_U8  i, u8WdrMode;
    ISP_BLACKLEVEL_S *pstBlackLevel = HI_NULL;
    isp_usr_ctx        *pstIspCtx     = HI_NULL;
    ...
    for (i = 0; i < pstRegCfg->cfg_num; i++) {
        BE_BlcDynaRegs(ViPipe, u8WdrMode, &pstRegCfg->alg_reg_cfg[i].stBeBlcCfg, pstBlackLevel);
        BE_BlcStaticRegs(u8WdrMode, &pstRegCfg->alg_reg_cfg[i].stBeBlcCfg);
        pstRegCfg->alg_reg_cfg[i].stWdrRegCfg.stSyncRegCfg.u16Offset0 = pstRegCfg->alg_reg_cfg[i].stBeBlcCfg.stWdrBlc[0].stUsrRegCfg.au16Blc[1];
    }
    pstRegCfg->cfg_key.bit1BeBlcCfg = 1;

    /* FE */
    FE_BlcDynaRegs(&pstRegCfg->alg_reg_cfg[0].stFeBlcCfg, pstBlackLevel);
    FE_BlcStaticRegs(&pstRegCfg->alg_reg_cfg[0].stFeBlcCfg);
    pstRegCfg->cfg_key.bit1FeBlcCfg = 1;
}
...
static HI_S32 ISP_BlcProcWrite(VI_PIPE ViPipe, hi_isp_ctrl_proc_write *pstProc)
{
...
HI_BOOL CheckWDRState(VI_PIPE ViPipe, isp_usr_ctx *pstIspCtx, ISP_BLACKLEVEL_S *pstBlackLevel)
{
    HI_U8   bWDREn;
    ...
    if (IS_2to1_WDR_MODE(pstIspCtx->sns_wdr_mode) || IS_3to1_WDR_MODE(pstIspCtx->sns_wdr_mode) || IS_4to1_WDR_MODE(pstIspCtx->sns_wdr_mode)) {
...
static HI_S32 ISP_BlcWdrModeSet(VI_PIPE ViPipe, HI_VOID *pRegCfg)
{
    ...
    for (i = 0; i < pstRegCfg->cfg_num; i++) {
        BE_BlcDynaRegs(ViPipe, pstIspCtx->sns_wdr_mode, &pstRegCfg->alg_reg_cfg[i].stBeBlcCfg, pstBlackLevel);
        pstRegCfg->alg_reg_cfg[i].stWdrRegCfg.stSyncRegCfg.u16Offset0 = pstRegCfg->alg_reg_cfg[i].stBeBlcCfg.stWdrBlc[0].stUsrRegCfg.au16Blc[1];
    }
    ...
    pstRegCfg->cfg_key.bit1BeBlcCfg = 1;
    pstRegCfg->cfg_key.bit1FeBlcCfg = 1;
    pstRegCfg->alg_reg_cfg[0].stFeBlcCfg.bReshDyna   = HI_TRUE;
    pstRegCfg->alg_reg_cfg[0].stFeBlcCfg.bReshStatic = HI_TRUE;
...
HI_S32 ISP_BlcRun(VI_PIPE ViPipe, const HI_VOID *pStatInfo, HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    if (hi_ext_system_dpc_static_defect_type_read(ViPipe) == 0) { // hot pixel
        if (pstIspCtx->linkage.defect_pixel) {
            if (pstBlackLevel->bPreDefectPixel == HI_FALSE) {
                for (i = 0; i < pstRegCfg->cfg_num; i++) {
                    pstRegCfg->alg_reg_cfg[i].stBeBlcCfg.stWbBlc.stStaticRegCfg.bBlcIn = HI_TRUE;
                    pstRegCfg->alg_reg_cfg[i].stBeBlcCfg.bReshStatic                   = HI_TRUE;
                }
            }
        } else if (pstBlackLevel->bPreDefectPixel) {
            for (i = 0; i < pstRegCfg->cfg_num; i++) {
                pstRegCfg->alg_reg_cfg[i].stBeBlcCfg.stWbBlc.stStaticRegCfg.bBlcIn = HI_FALSE;
                pstRegCfg->alg_reg_cfg[i].stBeBlcCfg.bReshStatic                   = HI_TRUE;
            }
        }

        pstBlackLevel->bPreDefectPixel = pstIspCtx->linkage.defect_pixel;
    }
    ...
    pstRegCfg->cfg_key.bit1FeBlcCfg = 1;
    pstRegCfg->cfg_key.bit1BeBlcCfg = 1;
    pstRegCfg->alg_reg_cfg[0].stFeBlcCfg.bFeBlcCfgChanged = HI_FALSE;
    ...
    if (sns_black_level->update == HI_TRUE) {
        if (pstIspCtx->linkage.snap_pipe_mode != ISP_SNAP_NONE) {
            if (ViPipe == pstIspCtx->linkage.picture_pipe_id) {
                MainPipe = pstIspCtx->linkage.preview_pipe_id;
                ISP_CHECK_PIPE(MainPipe);
                isp_sensor_get_blc(MainPipe, &sns_black_level);
            } else {
                isp_sensor_get_blc(ViPipe, &sns_black_level);
            }

            memcpy(&pstBlackLevel->au16BlackLevel[0], &sns_black_level->black_level[0], ISP_BAYER_CHN_NUM * sizeof(HI_U16));
        }
...
HI_S32 ISP_BlcCtrl(VI_PIPE ViPipe, HI_U32 u32Cmd, HI_VOID *pValue)
{
    ...
    switch (u32Cmd) {
        case ISP_WDR_MODE_SET :
            ...
            ISP_BlcWdrModeSet(ViPipe, (HI_VOID *)&pRegCfg->reg_cfg);
            break;
        case ISP_PROC_WRITE :
            ISP_BlcProcWrite(ViPipe, (hi_isp_ctrl_proc_write *)pValue);
            break;
        default :
            break;
    }
...
HI_S32 isp_alg_register_blc(VI_PIPE ViPipe)
{
    isp_usr_ctx *pstIspCtx = HI_NULL;
    isp_alg_node *pstAlgs = HI_NULL;

    ISP_GET_CTX(ViPipe, pstIspCtx);
    ISP_ALG_CHECK(pstIspCtx->alg_key.bit1_blc);
    pstAlgs = ISP_SearchAlg(pstIspCtx->algs);
    ISP_CHECK_POINTER(pstAlgs);

    pstAlgs->alg_type = ISP_ALG_BLC;
    pstAlgs->alg_func.pfn_alg_init = ISP_BlcInit;
    pstAlgs->alg_func.pfn_alg_run  = ISP_BlcRun;
    pstAlgs->alg_func.pfn_alg_ctrl = ISP_BlcCtrl;
    pstAlgs->alg_func.pfn_alg_exit = ISP_BlcExit;
