                  ...
                  HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    for (i = 0; i < pstRegCfg->cfg_num; i++) {
        CscDynaRegsConfig(pstCscAttr, &pstRegCfg->alg_reg_cfg[i].stCscCfg.stDynaRegCfg); /* Coefficiets config */
    }

    pstRegCfg->cfg_key.bit1CscCfg = HI_TRUE; /* Refresh flags are forced on */
...
HI_S32 isp_alg_register_csc(VI_PIPE ViPipe)
{
    ...
    ISP_GET_CTX(ViPipe, pstIspCtx);
    ISP_ALG_CHECK(pstIspCtx->alg_key.bit1_csc);
