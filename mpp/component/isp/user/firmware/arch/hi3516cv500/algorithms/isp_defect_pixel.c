...
static HI_VOID DpRegsInitialize(VI_PIPE ViPipe, isp_reg_cfg *pstRegCfg)
{
    ...
    for (i = 0; i < pstRegCfg->cfg_num; i++) {
        ...
        pstRegCfg->alg_reg_cfg[i].stDpRegCfg.bLut2SttEn = HI_TRUE;
    }
...
static HI_VOID DpcImageSize(VI_PIPE ViPipe, ISP_DEFECT_PIXEL_S *pstDp)
{
    ...
    pstDp->au16OffsetForSplit[u8BlockNum] = pstIspCtx->block_attr.frame_rect.width;
    pstDp->au16Offset[u8BlockNum]         = pstIspCtx->block_attr.frame_rect.width;
}
...
static HI_VOID DpExit(VI_PIPE ViPipe, ISP_DEFECT_PIXEL_S *pstDp, isp_reg_cfg *pstRegCfg)
{
    DpcDefCalibParam(pstDp);
...
static HI_S32 Dpc_Read_Calib_Num(VI_PIPE ViPipe, HI_U8 u8BlkNum, ISP_DEFECT_PIXEL_S *pstDp, isp_stat *pStatInfo)
{
    HI_U8  i;
    ...
    } else if (IS_OFFLINE_MODE(pstIspCtx->block_attr.running_mode)) {
        pstDp->u16BptCalibNum = MIN2(pStatInfo->dp_stat.defect_pixel_count[0], STATIC_DP_COUNT_NORMAL);

        ISP_CHECK_POINTER(pstDp->pau32BptCalibTable[0]);

        for (j = 0; j < pstDp->u16BptCalibNum; j++) {
            pstDp->pau32BptCalibTable[0][j] = pStatInfo->dp_stat.defect_pixel_lut[0][j];
        }
    } else if (IS_STRIPING_MODE(pstIspCtx->block_attr.running_mode)) {
        for (i = 0; i < u8BlkNum; i++) {
            ISP_CHECK_POINTER(pstDp->pau32BptCalibTable[i]);
            u16CntTemp = MIN2(pStatInfo->dp_stat.defect_pixel_count[i], STATIC_DP_COUNT_NORMAL);

            for (j = 0; j < u16CntTemp; j++) {
                u32BptValue = pStatInfo->dp_stat.defect_pixel_lut[i][j] + pstDp->au16Offset[i];
...
static HI_VOID Dpc_Calib_Success(VI_PIPE ViPipe, HI_U8 u8BlkNum, ISP_DEFECT_PIXEL_S *pstDp, isp_stat *pStatInfo)
{
    HI_U16 j;
...
static HI_VOID Dpc_Hot_Calib(VI_PIPE ViPipe, ISP_DEFECT_PIXEL_S  *pstDp, HI_U8 u8BlkNum, isp_reg_cfg *pstRegCfg, isp_stat *pStatInfo)
{
...
static HI_VOID Dpc_Dark_Calib(VI_PIPE ViPipe, ISP_DEFECT_PIXEL_S  *pstDp, HI_U8 u8BlkNum, isp_reg_cfg *pstRegCfg, isp_stat *pStatInfo)
{
    if (pstDp->u8FrameCnt < 9) {
        if (pstDp->u8FrameCnt == 0) {
            hi_ext_system_dpc_trigger_status_write(ViPipe, ISP_STATE_INIT);
...
static HI_VOID ISP_Dpc_StaticCalibration(VI_PIPE ViPipe, ISP_DEFECT_PIXEL_S  *pstDp, HI_U8 u8BlkNum, isp_reg_cfg *pstRegCfg, isp_stat *pStatInfo)
{
    HI_U8 i;
    isp_usr_ctx *pstIspCtx = HI_NULL;
...
static HI_VOID ISP_Dpc_Calib_Mode(VI_PIPE ViPipe, ISP_DEFECT_PIXEL_S *pstDp, isp_reg_cfg *pstRegCfg, isp_stat *pStatInfo)
{
    HI_U8 i;

    DpReadStaticCalibExtregs(ViPipe);
    ISP_Dpc_StaticCalibration(ViPipe, pstDp, pstRegCfg->cfg_num, pstRegCfg, pStatInfo);
...
static HI_VOID  SplitDpCorLut(VI_PIPE ViPipe, ISP_DEFECT_PIXEL_S  *pstDp, isp_reg_cfg *pstRegCfg, HI_U8 u8BlkNum)
{
    HI_S8  j;
...
static HI_VOID ISP_Dpc_Usr_Cfg(VI_PIPE ViPipe, ISP_DEFECT_PIXEL_S *pstDp, isp_reg_cfg *pstRegCfg)
{
    HI_U8 i;
...
static HI_VOID DpcResSwitch(VI_PIPE ViPipe, isp_reg_cfg *pstRegCfg)
{
    ISP_DEFECT_PIXEL_S  *pstDp     = HI_NULL;
...
static HI_VOID ISP_Dpc_Show_Mode(VI_PIPE ViPipe, isp_reg_cfg *pstRegCfg, isp_usr_ctx *pstIspCtx)
{
    HI_U8 i;
...
static HI_S32 ISP_Dynamic_set(HI_U32 u32Iso, ISP_DPC_DYNA_CFG_S *pstDpcHwCfg, ISP_DEFECT_PIXEL_S *pstDpcFwCfg)
{
    ...
    if (pstDpcFwCfg->bDynamicManual) {
        ...
        u16Strength     = (HI_U16)LinearInter(u32Iso, \
                                              g_au32IsoLut[u8IsoIndexLower], (HI_S32)pstDpc->strength[u8IsoIndexLower], \
                                              g_au32IsoLut[u8IsoIndexUpper], (HI_S32)pstDpc->strength[u8IsoIndexUpper]);
        u16BlendRatio   = (HI_U16)LinearInter(u32Iso, \
                                              g_au32IsoLut[u8IsoIndexLower], (HI_S32)pstDpc->blend_ratio[u8IsoIndexLower], \
                                              g_au32IsoLut[u8IsoIndexUpper], (HI_S32)pstDpc->blend_ratio[u8IsoIndexUpper]);
    }
...
static HI_S32 ISP_Dpc_Normal_Mode(VI_PIPE ViPipe, ISP_DEFECT_PIXEL_S *pstDp, isp_reg_cfg *pstRegCfg, isp_usr_ctx *pstIspCtx)
{
    HI_U8 i;
    ...
    for (i = 0; i < pstRegCfg->cfg_num; i++) {
        ISP_Dynamic_set(pstIspCtx->linkage.iso, &pstRegCfg->alg_reg_cfg[i].stDpRegCfg.stDynaRegCfg, pstDp);
        pstRegCfg->alg_reg_cfg[i].stDpRegCfg.stDynaRegCfg.bResh = HI_TRUE;
    }
                 ...
                 HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    if ((pstDp->bStaCalibrationEn == HI_FALSE) && (pstDp->u8CalibStarted == 1)) { /* quit calibration */
        DpExit(ViPipe, pstDp, pstRegCfg);
        if (IS_OFFLINE_MODE(pstIspCtx->block_attr.running_mode) || \
            IS_STRIPING_MODE(pstIspCtx->block_attr.running_mode)) {
    ...
    if (pstDp->bStaCalibrationEn == HI_TRUE) { /* calibration mode */
        ISP_Dpc_Calib_Mode(ViPipe, pstDp, pstRegCfg, (isp_stat *)pStatInfo);
    ...
    if (pstDp->bStaicShow == HI_TRUE) { /* highlight static defect pixels mode */
...
HI_S32 ISP_DpCtrl(VI_PIPE ViPipe, HI_U32 u32Cmd, HI_VOID *pValue)
{
    ...
    switch (u32Cmd) {
        ...
        case ISP_CHANGE_IMAGE_MODE_SET:
            ISP_REGCFG_GET_CTX(ViPipe, pRegCfg);
            ISP_CHECK_POINTER(pRegCfg);
            DpcResSwitch(ViPipe, &pRegCfg->reg_cfg);
            break;
        default :
            break;
    }
...
HI_S32 isp_alg_register_dpc(VI_PIPE ViPipe)
{
    ...
    ISP_GET_CTX(ViPipe, pstIspCtx);
    ISP_ALG_CHECK(pstIspCtx->alg_key.bit1_dp);
