...
HI_S32 GammaCtxInit(VI_PIPE ViPipe)
{
    ...
    if (pstGamma == HI_NULL) {
        ...
        if (pstGamma == HI_NULL) {
            ISP_ERR_TRACE("Isp[%d] GammaCtx malloc memory failed!\n", ViPipe);
...
static HI_VOID GammaRegsInitialize(VI_PIPE ViPipe, isp_reg_cfg *pstRegCfg)
{
    HI_U16 i;
    ...
    for (i = 0; i < pstRegCfg->cfg_num; i++) {
        GammaUsrRegsInit(&pstRegCfg->alg_reg_cfg[i].stGammaCfg.stUsrRegCfg, pstGammaCtx->pu16GAMMA, pstGammaCtx);
        pstRegCfg->alg_reg_cfg[i].stGammaCfg.bGammaEn   = HI_TRUE;
        pstRegCfg->alg_reg_cfg[i].stGammaCfg.bLut2SttEn = HI_TRUE;
    }
...
static HI_S32 GammaInitialize(VI_PIPE ViPipe)
{
    ...
    if (pstGammaCtx->pu16GAMMA == HI_NULL) {
        ...
        if (pstGammaCtx->pu16GAMMA == HI_NULL) {
            ISP_ERR_TRACE("malloc pstGammaCtx->pau16GAMMA failed!\n");
            return HI_FAILURE;
        }
    }
    ...
    if (pstGammaCtx->pu16ExtGAMMA == HI_NULL) {
        ...
        if (pstGammaCtx->pu16ExtGAMMA == HI_NULL) {
            ISP_ERR_TRACE("malloc pstGammaCtx->pau16ExtGAMMA failed!\n");
    ...
    if (sns_dft->key.bit1_gamma) {
        ...
        memcpy(pstGammaCtx->pu16ExtGAMMA, sns_dft->gamma->gamma_lut, GAMMA_EXT_NODE_NUMBER * sizeof(HI_U16));
...
static HI_VOID ISP_GammaWdrModeSet(VI_PIPE ViPipe, HI_VOID *pRegCfg)
{
    ...
    for (i = 0; i < pstRegCfg->cfg_num; i++) {
        ...
        pstRegCfg->alg_reg_cfg[i].stGammaCfg.stUsrRegCfg.bSwitchMode          = HI_TRUE;
        pstRegCfg->alg_reg_cfg[i].stGammaCfg.stUsrRegCfg.bSwitchLut2SttRegNew = HI_TRUE;
        pstRegCfg->alg_reg_cfg[i].stGammaCfg.stUsrRegCfg.u8SwitchRegNewCnt    = 0;
    }
}
                    ...
                    HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    if (HI_NULL == pstGammaCtx->pu16GAMMA || HI_NULL == pstGammaCtx->pu16ExtGAMMA) {
        ISP_ERR_TRACE("Gamma lutCtx is empty!\n");
        return HI_FAILURE;
    }
...
HI_S32 isp_alg_register_gamma(VI_PIPE ViPipe)
{
    ...
    ISP_GET_CTX(ViPipe, pstIspCtx);
    ISP_ALG_CHECK(pstIspCtx->alg_key.bit1_gamma);
