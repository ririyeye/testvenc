...
HI_S32 ISP_AfExtRegsDefault(VI_PIPE ViPipe)
{
    ...
    hi_ext_af_crop_hsize_write(ViPipe, pstIspCtx->block_attr.frame_rect.width / 8 * 8);
    hi_ext_af_crop_vsize_write(ViPipe, pstIspCtx->block_attr.frame_rect.height / 2 * 2);
    ...
    hi_ext_af_bayermode_write(ViPipe, (HI_U8)u32Cfa);
    ...
    g_stFocusCfg[ViPipe].stConfig.u16Hsize = pstIspCtx->block_attr.frame_rect.width;
    g_stFocusCfg[ViPipe].stConfig.u16Vsize = pstIspCtx->block_attr.frame_rect.height;
...
static HI_VOID ISP_AfBlkCfg(VI_PIPE ViPipe, isp_reg_cfg *pstRegCfg, isp_usr_ctx *pstIspCtx)
{
    HI_U8  i;
    HI_U8  u8BlockNum   = pstRegCfg->cfg_num;
    ...
    for (i = 0; i < u8BlockNum; i++) {
        pstAFdefaultCfg   = &pstRegCfg->alg_reg_cfg[i].stBEAfRegCfg;
        ...
        pstRegCfg->kernel_reg_cfg.alg_kernel_cfg[i].stBeAFZoneCfg.u32Colum     =  pstAFdefaultCfg->u16WindowHnum;
        pstRegCfg->kernel_reg_cfg.alg_kernel_cfg[i].stBeAFZoneCfg.u32Row       =  pstAFdefaultCfg->u16WindowVnum;
        ...
        if (u8BlockNum == 1) {
            ...
            pstAFdefaultCfg->u16CropPosY        = stBlockRect.y;
            pstAFdefaultCfg->u16CropVsize       = stBlockRect.height;

            if (i == 0) {
                if (u8BlockNum > 1) {
                    pstAFdefaultCfg->u16CropPosX   = 0;
                    pstAFdefaultCfg->u16CropHsize  = stBlockRect.width - u16Overlap;
                } else {
                    pstAFdefaultCfg->u16CropPosX  = 0;
                    pstAFdefaultCfg->u16CropHsize = stBlockRect.width;
                }
                ...
                pstAFdefaultCfg->u16CropPosX   = u16Overlap;
                pstAFdefaultCfg->u16CropHsize  = stBlockRect.width - (u16Overlap << 1);
            }
        }
    }
}

HI_S32 ISP_SetAfBeDefault(VI_PIPE ViPipe, isp_reg_cfg *pstRegCfg)
{
    HI_U8 u8BlockNum = 0;
    HI_U8 i;
    HI_U8 u8I, u8Shift0;
    ...
    for (i = 0; i < u8BlockNum; i++) {
        ...
        if ((pstIspCtx->block_attr.frame_rect.height < AF_MIN_HEIGHT) ||
            (pstIspCtx->block_attr.frame_rect.width < AF_MIN_WIDTH)) {
        ...
        if (pstIspCtx->isp_yuv_mode == HI_TRUE) {
        ...
        pstRegCfg->cfg_key.bit1AfBeCfg        = 1;
        pstRegCfg->kernel_reg_cfg.cfg_key.bit1BeAfCfg = 1;
    }
...
HI_S32 ISP_AfBeCfgRegs(VI_PIPE ViPipe, isp_reg_cfg *pstRegCfg)
{
    HI_U8 u8BlockNum = 0;
    HI_U8 u8WindowHnum;
    HI_U8 i;
...
static HI_VOID ISP_AfImageResWrite(VI_PIPE ViPipe, isp_reg_cfg *pstRegCfg)
{
    HI_U8 i;
    isp_usr_ctx *pstIspCtx = HI_NULL;
    ISP_GET_CTX(ViPipe, pstIspCtx);
    ...
    for (i = 0; i < pstRegCfg->cfg_num; i++) {
        pstRegCfg->alg_reg_cfg[i].stBEAfRegCfg.u32UpdateIndex++;
    }

    pstRegCfg->cfg_key.bit1AfFeCfg = 1;
}
...
HI_S32 ISP_AfInit(VI_PIPE ViPipe, HI_VOID *pRegCfg)
{
    ...
    ISP_SetAfBeDefault(ViPipe, (isp_reg_cfg *)pRegCfg);
                 ...
                 HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    if (hi_ext_af_set_flag_read(ViPipe) == HI_EXT_AF_SET_FLAG_ENABLE) {
        hi_ext_af_set_flag_write(ViPipe, HI_EXT_AF_SET_FLAG_DISABLE);
        ISP_AfBeCfgRegs(ViPipe, (isp_reg_cfg *)pRegCfg);
    }
...
HI_S32 ISP_AfCtrl(VI_PIPE ViPipe, HI_U32 u32Cmd, HI_VOID *pValue)
{
    ...
    switch (u32Cmd) {
        case ISP_WDR_MODE_SET :
            ...
            ISP_AfWdrModeSet(ViPipe, (HI_VOID *)&pRegCfg->reg_cfg);
            break;
        case ISP_CHANGE_IMAGE_MODE_SET:
            ISP_REGCFG_GET_CTX(ViPipe, pRegCfg);
            ISP_CHECK_POINTER(pRegCfg);
            ISP_AfImageResWrite(ViPipe, &pRegCfg->reg_cfg);
            break;
        default :
            break;
    }
...
HI_S32 isp_alg_register_af(VI_PIPE ViPipe)
{
    isp_usr_ctx *pstIspCtx = HI_NULL;
    isp_alg_node *pstAlgs = HI_NULL;

    ISP_GET_CTX(ViPipe, pstIspCtx);
    ISP_ALG_CHECK(pstIspCtx->alg_key.bit1_af);

    pstAlgs = ISP_SearchAlg(pstIspCtx->algs);
    ISP_CHECK_POINTER(pstAlgs);

    pstAlgs->alg_type = ISP_ALG_AF;
    pstAlgs->alg_func.pfn_alg_init = ISP_AfInit;
    pstAlgs->alg_func.pfn_alg_run  = ISP_AfRun;
    pstAlgs->alg_func.pfn_alg_ctrl = ISP_AfCtrl;
    pstAlgs->alg_func.pfn_alg_exit = ISP_AfExit;
