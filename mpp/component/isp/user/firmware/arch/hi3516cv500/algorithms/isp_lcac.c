...
static HI_S32 local_cac_dyna_fw(VI_PIPE ViPipe, HI_U8 u8CurBlk, ISP_LCAC_S *pstLocalCAC,
                                ISP_LOCAL_CAC_DYNA_CFG_S *pstLCacDynaRegCfg, HI_U8 u8WDRMode)
{
...
static HI_VOID ISP_LCacWdrModeSet(VI_PIPE ViPipe, HI_VOID *pRegCfg)
{
    ...
    for (i = 0; i < pstRegCfg->cfg_num; i++) {
        au32UpdateIdx[i] = pstRegCfg->alg_reg_cfg[i].stLCacRegCfg.stUsrRegCfg.u32UpdateIndex;
    }

    ISP_LCacInit(ViPipe, pRegCfg);

    for (i = 0; i < pstRegCfg->cfg_num; i++) {
        pstRegCfg->alg_reg_cfg[i].stLCacRegCfg.stUsrRegCfg.u32UpdateIndex = au32UpdateIdx[i] + 1;
    }
}
                   ...
                   HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    for (i = 0; i < pstRegCfg->cfg_num; i++) {
        local_cac_dyna_fw(ViPipe, i, pstLocalCAC, &pstRegCfg->alg_reg_cfg[i].stLCacRegCfg.stDynaRegCfg,
                          pstIspCtx->sns_wdr_mode);
    }
...
HI_S32 ISP_LCacCtrl(VI_PIPE ViPipe, HI_U32 u32Cmd, HI_VOID *pValue)
{
    ...
    switch (u32Cmd) {
        case ISP_WDR_MODE_SET :
            ...
            ISP_LCacWdrModeSet(ViPipe, (HI_VOID *)&pRegCfg->reg_cfg);
            break;
        case ISP_PROC_WRITE:
            break;
        default :
...
HI_S32 isp_alg_register_lcac(VI_PIPE ViPipe)
{
    ...
    ISP_GET_CTX(ViPipe, pstIspCtx);
    ISP_ALG_CHECK(pstIspCtx->alg_key.bit1_lcac);
