...
#ifndef __HuaweiLite__
    #define DUMP_DATA_PATH "./"
#else
    #define DUMP_DATA_PATH "/sharefs"
#endif
...
HI_S32 DumpLinearBayer(VI_PIPE ViPipe, HI_U32 u32Nbit, COMPRESS_MODE_E enCompressMode, HI_U32 u32Cnt, HI_U32 u32ByteAlign, HI_U32 u32RatioShow)
{
    ...
    if (0 == u32RatioShow)
    {
        snprintf(szYuvName, 256, DUMP_DATA_PATH"/vi_Pipe_%d_%d_%d_%d_%dbits_%s_%d_%d.raw", ViPipe,
    ...
    else
    {
        ...
        snprintf(szYuvName, 256, DUMP_DATA_PATH"/HisiRAW_%lld_%d_%d_%d_%dbits_%s_Linear_Ratio%d_ISO%d_AG%d_DG%d_IspDG%d_Exp%dus_Cnt%d.raw",
...
getframeok:
    ...
    if (0 == u32RatioShow)
    {
        snprintf(szYuvName, 256, DUMP_DATA_PATH"/vi_Pipe_%d_%d_%d_%d_%dbits_%s_%d_lineWDR.raw", ViPipeId[0],
    ...
    else
    {
        ...
        if (4 == u32PipeNum)
        {
            snprintf(szYuvName, 256, DUMP_DATA_PATH"/HisiRAW_%lld_%d_%d_%d_%dbits_%s_WDR_Ratio(0)%d_Ratio(1)%d_Ratio(2)%d_ISO%d_AG%d_DG%d_IspDG%d_Exp%dus_Cnt%d.raw",
        ...
        {
            snprintf(szYuvName, 256, DUMP_DATA_PATH"/HisiRAW_%lld_%d_%d_%d_%dbits_%s_WDR_Ratio(0)%d_Ratio(1)%d_ISO%d_AG%d_DG%d_IspDG%d_Exp%dus_Cnt%d.raw",
                    pastFrame[0][0].stVFrame.u64PTS, pstIspFrameInfo->u32SensorID, pastFrame[0][0].stVFrame.u32Width,  pastFrame[0][0].stVFrame.u32Height, u32Nbit,
        ...
        {
            snprintf(szYuvName, 256, DUMP_DATA_PATH"/HisiRAW_%lld_%d_%d_%d_%dbits_%s_WDR_Ratio(0)%d_ISO%d_AG%d_DG%d_IspDG%d_Exp%dus_Cnt%d.raw",
                    pastFrame[0][0].stVFrame.u64PTS, pstIspFrameInfo->u32SensorID, pastFrame[0][0].stVFrame.u32Width,  pastFrame[0][0].stVFrame.u32Height, u32Nbit,
                    IspBayerFormatToString(stPubAttr.enBayer), pstIspFrameInfo->au32Ratio[0], pstIspFrameInfo->u32ISO, pstIspFrameInfo->u32Again,
